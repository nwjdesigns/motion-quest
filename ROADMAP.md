...
| 2026-07-03 | **Homepage marquee + particle polish.** Fixed TopBar/FooterBar `position: fixed` bug (entrance choreography wrapper's `transform: translateY(0px)` created a containing block; now returns `{}` when animation complete). **Marquee drift:** each experiment node drifts left-to-right at its own speed (0.5x-1.5x) with per-node phase offset for even distribution on load; nodes wrap via `wrapX` modular arithmetic in `ExperimentNode`; `MarqueeClock` drives a shared offset ref. **Billboard:** nodes always face camera (`quaternion.copy`). **Live connecting lines:** shared `Float32Array` ref written by each node, read by `ConnectingLines` every frame. **Ambient particles:** 2000 count (was 200), smaller scale (max 0.04), wider spread (18), slower drift with larger amplitudes. 356 tests |
...
- **Projection mapping as an output medium** — projecting Cavalry/AE scenes onto physical surfaces instead of rendering flat 9:16. Hardware research done 2026-08-15 (`docs/sessions/2026-08-15-projection-mapping-projector-research.md`): buy used ex-corporate business laser with real lens shift, not new consumer; Epson EB-X49 ruled out (native XGA 4:3 gives ~590k effective pixels on 16:9). Cheapest validation is a day's projector rental in Cape Town before buying anything. Unanswered fork: interior objects vs building facade, which is a 6,000+ lumen and very different budget. Nothing bought, no build scope.
